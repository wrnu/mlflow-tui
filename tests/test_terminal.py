...
def test_tty_defaults_mouse_on(monkeypatch) -> None:
...
def test_non_tty_and_dumb_term_default_mouse_off(monkeypatch) -> None:
    monkeypatch.delenv("MLFLOW_TUI_MOUSE", raising=False)
...
def test_install_quiet_driver_blocks_reenable() -> None:
    driver = _Driver()
    install_quiet_driver(driver, allow_mouse=lambda: False)
    ...
    driver._request_terminal_sync_mode_support()
    assert "query-sync" not in driver.writes
    driver._enable_bracketed_paste()
    assert driver.paste_calls == 1
    driver.process_message(InBandWindowResize())
    assert driver.processed == []
    driver.process_message("other")
    assert driver.processed == ["other"]


def test_keep_terminal_quiet_always_drops_optional_probes() -> None:
    driver = _Driver()
    keep_terminal_quiet(driver, allow_mouse=True)
    blob = "".join(driver.writes)
    assert "1004l" in blob
    assert "1016l" in blob
    assert "25l" in blob
    assert "1003l" not in blob
    assert "2004l" not in blob
    assert driver._in_band_window_resize is False
